...
const modes = [
  { label: 'Solo', short: 'Your pace, your rules.', image: 'https://images.unsplash.com/photo-1501555088652-021faa106b9b?w=800&q=85', icon: MapPin, accent: 'from-amber-500/80' },
  { label: 'Family', short: 'Memories for everyone.', image: 'https://images.unsplash.com/photo-1476514525535-07fb3b4ae5f1?w=800&q=85', icon: Users, accent: 'from-sky-500/80' },
  { label: 'Adventure', short: 'Push every limit.', image: 'https://images.unsplash.com/photo-1464822759023-fed622ff2c3b?w=800&q=85', icon: Mountain, accent: 'from-emerald-500/80' },
  { label: 'Business', short: 'Effortless corporate travel.', image: 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=800&q=85', icon: Briefcase, accent: 'from-violet-500/80' },
]
            ...
            {modes.map((mode, i) => {
              ...
              return (
                <button key={mode.label} onClick={() => setActive(i)} className={`group relative flex-1 lg:flex-none overflow-hidden rounded-2xl cursor-pointer text-left ${isActive ? 'ring-2 ring-black/20' : ''}`} style={{ minHeight: isActive ? '160px' : '110px', transition: 'min-height 0.4s ease' }}>
                  <img src={mode.image} alt={mode.label} loading="lazy" className={`absolute inset-0 w-full h-full object-cover transition-transform duration-700 ${isActive ? 'scale-[1.04]' : 'scale-100 group-hover:scale-[1.02]'}`} />
