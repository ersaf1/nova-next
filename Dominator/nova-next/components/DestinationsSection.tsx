...
const staticDestinations: Destination[] = [
  { id: 1, city: 'Tokyo', country: 'Japan', tagline: 'Where tradition meets tomorrow', price: 'From $899', image: '', tag: 'Popular', rating: 4.9, duration: '7–10 days' },
  { id: 2, city: 'Santorini', country: 'Greece', tagline: 'Sunsets worth crossing oceans for', price: 'From $1,199', image: '', tag: 'Trending', rating: 4.8, duration: '5–8 days' },
  { id: 3, city: 'New York', country: 'USA', tagline: 'The city that never stops inspiring', price: 'From $749', image: '', tag: null, rating: 4.7, duration: '4–7 days' },
  { id: 4, city: 'Bali', country: 'Indonesia', tagline: 'Spirit, serenity, and soul', price: 'From $699', image: '', tag: "Editor's Pick", rating: 4.9, duration: '7–14 days' },
  { id: 5, city: 'Paris', country: 'France', tagline: 'Romance written in stone and light', price: 'From $1,049', image: '', tag: null, rating: 4.8, duration: '4–6 days' },
  { id: 6, city: 'Rome', country: 'Italy', tagline: 'History, beauty, and culinary magic', price: 'From $949', image: '', tag: 'Popular', rating: 4.8, duration: '5–7 days' },
  { id: 7, city: 'Reykjavik', country: 'Iceland', tagline: 'Land of fire, ice, and aurora skies', price: 'From $1,249', image: '', tag: 'Trending', rating: 4.9, duration: '6–9 days' },
  { id: 8, city: 'Machu Picchu', country: 'Peru', tagline: 'Ancient mystery high in the clouds', price: 'From $1,399', image: '', tag: null, rating: 4.9, duration: '8–12 days' },
]
...
const Lightbox: React.FC<LightboxProps> = ({ dest, onClose, onPrev, onNext, saved, onToggleSave }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center" style={{ background: 'rgba(0,0,0,0.92)' }} onClick={onClose}>
      {dest.image && <div className="absolute inset-0" style={{ backgroundImage: `url(${dest.image})`, backgroundSize: 'cover', backgroundPosition: 'center', filter: 'blur(40px) brightness(0.25)', transform: 'scale(1.1)' }} />}
      <div className="relative w-full max-w-5xl mx-4 sm:mx-6 rounded-3xl overflow-hidden flex flex-col sm:flex-row" style={{ maxHeight: '90vh', boxShadow: '0 40px 120px rgba(0,0,0,0.8)' }} onClick={e => e.stopPropagation()}>
        ...
        >
          {/* Specular highlight line at top of panel */}
          ...
          <div>
            <div className="hidden sm:flex items-center gap-1 mb-2"><MapPin className="w-3.5 h-3.5 text-white/40" /><span className="text-white/40 text-xs font-bold uppercase tracking-widest">{dest.country}</span></div>
            <h2 className="hidden sm:block text-white text-4xl font-bold mb-3" style={{ letterSpacing: '-0.03em' }}>{dest.city}</h2>
            <p className="text-white/60 text-base leading-relaxed mb-8">{dest.tagline}</p>
            <div className="grid grid-cols-3 gap-4 mb-8">
              <div style={{ background: 'rgba(255,255,255,0.07)', borderRadius: '1rem', border: '1px solid rgba(255,255,255,0.10)' }} className="p-4"><div className="flex items-center gap-1 mb-1"><Star className="w-3.5 h-3.5 fill-white text-white" /><span className="text-white font-bold text-lg" style={{ letterSpacing: '-0.02em' }}>{dest.rating}</span></div><p className="text-white/40 text-[10px] font-bold uppercase tracking-wider">Rating</p></div>
              <div style={{ background: 'rgba(255,255,255,0.07)', borderRadius: '1rem', border: '1px solid rgba(255,255,255,0.10)' }} className="p-4"><div className="flex items-center gap-1 mb-1"><Clock className="w-3.5 h-3.5 text-white/40" /></div><p className="text-white font-bold text-sm mb-1">{dest.duration}</p><p className="text-white/40 text-[10px] font-bold uppercase tracking-wider">Duration</p></div>
              <div style={{ background: 'rgba(255,255,255,0.07)', borderRadius: '1rem', border: '1px solid rgba(255,255,255,0.10)' }} className="p-4"><p className="text-white font-bold text-lg leading-tight mb-1" style={{ letterSpacing: '-0.02em' }}>{dest.price.replace('From ', '')}</p><p className="text-white/40 text-[10px] font-bold uppercase tracking-wider">From / person</p></div>
            </div>
          </div>
          ...
          <ScrollReveal staggerChildren={true} animation="slide-up" delay={0.1}>
            <div className="grid grid-cols-3 grid-rows-2 gap-3 md:gap-4" style={{ gridTemplateRows: '280px 280px' }}>
              {destinations.slice(0, 6).map((dest, i) => {
                // Row 1: [wide, small, small] — index 0 spans 2 cols
                // Row 2: [small, small, wide] — index 5 spans 2 cols
                const isWide = i === 0 || i === 5
                return (
                  <div key={dest.id} className={`group relative overflow-hidden rounded-2xl cursor-pointer transition-all duration-500 hover:shadow-2xl hover:-translate-y-1.5 ${isWide ? 'col-span-2' : 'col-span-1'}`} onClick={() => setLightboxIndex(i)}>
                    {dest.image ? (
                      <img src={dest.image} alt={`${dest.city}, ${dest.country}`} loading="lazy" className="w-full h-full object-cover img-smooth-zoom" />
                    ) : null}
                    ...
                    <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                      <Link href={`/destinations/${dest.id}`} onClick={e => e.stopPropagation()} className="w-12 h-12 rounded-full bg-white/15 backdrop-blur-sm border border-white/30 flex items-center justify-center hover:bg-white/30 transition-colors">
