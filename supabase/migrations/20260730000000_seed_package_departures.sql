-- ============================================================
-- NOVA Migration: Seed PackageDeparture untuk semua package valid
-- ============================================================

-- Hapus jadwal lama untuk package valid agar tidak duplikat
...
SELECT p.id, d.start_date, d.start_date + INTERVAL '7 days', d.cap, d.slots, p.price, d.status
FROM "Package" p,
(VALUES
...
) AS d(start_date, cap, slots, status)
WHERE p.slug = 'bali-paradise-escape';

-- ─── Japan Cherry Blossom Tour (12H11M, City) ───────────────
INSERT INTO "PackageDeparture" ("packageId","startDate","endDate","capacity","remainingSlots","price","status")
SELECT p.id, d.start_date, d.start_date + INTERVAL '11 days', d.cap, d.slots, p.price, d.status
FROM "Package" p,
(VALUES
...
INSERT INTO "PackageDeparture" ("packageId","startDate","endDate","capacity","remainingSlots","price","status")
SELECT p.id, d.start_date, d.start_date + INTERVAL '6 days', d.cap, d.slots, p.price, d.status
FROM "Package" p,
(VALUES
...
INSERT INTO "PackageDeparture" ("packageId","startDate","endDate","capacity","remainingSlots","price","status")
SELECT p.id, d.start_date, d.start_date + INTERVAL '8 days', d.cap, d.slots, p.price, d.status
FROM "Package" p,
(VALUES
...
INSERT INTO "PackageDeparture" ("packageId","startDate","endDate","capacity","remainingSlots","price","status")
SELECT p.id, d.start_date, d.start_date + INTERVAL '5 days', d.cap, d.slots, p.price, d.status
FROM "Package" p,
(VALUES
