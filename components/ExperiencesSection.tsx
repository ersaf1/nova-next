...
const modes = [
  ...
  {
    label: 'Business',
    short: 'Effortless corporate travel.',
    image: 'https://images.unsplash.com/photo-1436491865332-7a61a109cc05?w=800&auto=format&q=75',
    icon: Briefcase,
    accent: 'from-neutral-900/80',
  },
]
    ...
    <section id="experiences" className="bg-[#F5F5F5] px-4 sm:px-6 py-20">
      <div className="max-w-[88rem] mx-auto">
        <div className="mb-10 flex items-end justify-between">
          ...
          <Link href="/search" className="hidden sm:inline-flex items-center gap-2 text-black/50 text-sm font-medium hover:text-black transition-colors group">
            Explore all <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
          </Link>
        </div>
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-4 md:gap-6">
          {/* Mode buttons — stagger animated individually */}
            ...
            {modes.map((mode, i) => {
              ...
              return (
                <button
                  key={mode.label}
                  onClick={() => setActive(i)}
                  className={`group relative flex-1 lg:flex-none overflow-hidden rounded-2xl cursor-pointer text-left ${isActive ? 'ring-2 ring-black/20' : ''}`}
                  style={{ minHeight: isActive ? '160px' : '110px', transition: 'min-height 0.4s ease' }}
                >
                  <img
                    src={mode.image}
                    alt={mode.label}
            ...
            {modes.map((mode, i) => (
              ...
              >
                ...
                <div className="absolute bottom-0 left-0 right-0 p-8">
                  ...
                  >
                    <span className="w-10 h-10 rounded-full bg-white/20 backdrop-blur border border-white/30 flex items-center justify-center hover:bg-white/30 transition-colors">
